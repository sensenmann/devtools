...
import { getLatestDueTime, getNextDueTime } from "../src/scheduler.ts";
...
test("scheduler resolves hourly jobs to the next local hour", () => {
  const now = new Date(2026, 4, 21, 14, 37, 12, 0);
  const due = getNextDueTime({ kind: "hourly" }, now);
  assert.ok(due);
  assert.equal(due.getHours(), 15);
  assert.equal(due.getMinutes(), 0);
});

test("scheduler resolves daily jobs to the next local occurrence", () => {
  const schedule: ScheduleDefinition = { kind: "daily", time: "16:45" };

  const sameDay = getNextDueTime(schedule, new Date(2026, 4, 21, 12, 0, 0, 0));
  assert.ok(sameDay);
  assert.equal(sameDay.getDate(), 21);
  assert.equal(sameDay.getHours(), 16);
  assert.equal(sameDay.getMinutes(), 45);

  const nextDay = getNextDueTime(schedule, new Date(2026, 4, 21, 18, 0, 0, 0));
  assert.ok(nextDay);
  assert.equal(nextDay.getDate(), 22);
  assert.equal(nextDay.getHours(), 16);
  assert.equal(nextDay.getMinutes(), 45);
});

test("scheduler resolves weekly jobs to the next local weekday occurrence", () => {
  const schedule: ScheduleDefinition = { kind: "weekly", weekday: "friday", time: "09:30" };

  const sameWeek = getNextDueTime(schedule, new Date(2026, 4, 21, 12, 0, 0, 0));
  assert.ok(sameWeek);
  assert.equal(sameWeek.getDay(), 5);
  assert.equal(sameWeek.getDate(), 22);
  assert.equal(sameWeek.getHours(), 9);
  ...
  const nextWeek = getNextDueTime(schedule, new Date(2026, 4, 22, 12, 0, 0, 0));
  assert.ok(nextWeek);
  assert.equal(nextWeek.getDay(), 5);
  assert.equal(nextWeek.getDate(), 29);
  assert.equal(nextWeek.getHours(), 9);
  assert.equal(nextWeek.getMinutes(), 30);
});
